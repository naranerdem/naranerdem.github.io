...
export class AdditionalClassAdmissionError extends Error {
  constructor(public readonly code: "forbidden" | "not_found" | "invalid" | "unsupported" | "source_fully_paid" | "conflict") {
...
type SourceRow = {
  ...
  facebookName: string | null; homeAddress: string; sourcePaymentPlanCode: string | null;
};

type TargetRow = {
  classSessionId: string; stageCode: "stage_1" | "stage_2" | "stage_3"; classIsTest: number;
  offeringIsTest: number; yearIsTest: number; status: string; twoEnabled: number | null;
...
async function targetForMutation(env: WorkerEnv, source: SourceRow, targetClassSessionId: string): Promise<TargetRow> {
  const row = await env.DB.prepare(`SELECT class_session.id AS classSessionId, class_session.stage_code AS stageCode,
      ...
      academic_year.is_test AS yearIsTest, class_session.status, pricing.two_installment_enabled AS twoEnabled,
...
async function sourceFinancialSupport(env: WorkerEnv, source: SourceRow, basisPoints: number) {
  ...
  if (source.sourcePaymentPlanCode !== "two_installment") {
    const totalScheduled = installments.results.reduce((sum, item) => sum + number(item.amountMnt), 0);
    const totalAllocated = installments.results.reduce((sum, item) => sum + number(item.allocatedAmountMnt), 0);
    throw new AdditionalClassAdmissionError(totalScheduled > 0 && totalAllocated >= totalScheduled ? "source_fully_paid" : "unsupported");
  }
  if (installments.results.length !== 2 || !installments.results.some((item) => item.installmentNumber === 1) || !installments.results.some((item) => item.installmentNumber === 2)) {
  ...
  const effective = await effectiveInstallmentsForRows(env.DB, installments.results.map((item) => ({ ...item, amountMnt: number(item.amountMnt), allocatedAmountMnt: number(item.allocatedAmountMnt) })));
  const first = effective.find((item) => item.installmentNumber === 1);
  const second = effective.find((item) => item.installmentNumber === 2);
  if (!first || !second || number(first.allocatedAmountMnt) < number(first.effectiveAmountMnt)) throw new AdditionalClassAdmissionError("unsupported");
  ...
  const secondOutstanding = Math.max(0, number(second.effectiveAmountMnt) - number(second.allocatedAmountMnt));
  if (proposed > secondOutstanding) throw new AdditionalClassAdmissionError("unsupported");
...
export async function createAdditionalClassAdmission(env: WorkerEnv, actor: StaffPrincipal, input: {
  ...
  childAcknowledged: boolean; idempotencyKey: string;
}, nowDate = new Date()) {
  if (!hasStaffCapability(actor, "registration.manage")) throw new AdditionalClassAdmissionError("forbidden");
  if (!input.parentAcknowledged || !input.childAcknowledged || input.paymentPlanCode !== "two_installment" || !input.idempotencyKey) {
    throw new AdditionalClassAdmissionError("invalid");
  }
  ...
  const replay = await env.DB.prepare(`SELECT additional_class_admission.id AS admissionId,
    ...
    WHERE additional_class_admission.idempotency_key = ?`).bind(input.idempotencyKey).first<{
      ...
      status: "pending_confirmation" | "confirmed" | "cancelled" | "expired"; email: string; targetClassSessionId: string;
      paymentDeadlineAt: string | null;
  }>();
  if (replay) {
    if (replay.sourceChildId !== input.registrationDraftChildId || replay.staffAccountId !== actor.staffAccountId || replay.targetClassSessionId !== input.targetClassSessionId) {
  ...
  const targetTotal = number(target.firstAmountMnt) + number(target.secondAmountMnt);
  const targetAwardMnt = discountAmountMnt(targetTotal, policy.familyMultiChildBasisPoints);
  if (targetAwardMnt <= 0 || targetAwardMnt > number(target.secondAmountMnt)) throw new AdditionalClassAdmissionError("unsupported");
  ...
  try {
    const created = await createRegistrationDraft(env, {
      ...
      children: [{ surname: source.surname, givenName: source.givenName, gender: source.gender, dateOfBirth: source.dateOfBirth,
        ...
        selectedClassSessionId: target.classSessionId, paymentPlanCode: "two_installment" }],
