...
export interface PaymentConfirmedChild {
  childName: string;
  classLabel: string;
  eventAllocatedAmountMnt: number;
...
export function paymentConfirmedTemplate(input: {
  ...
  const items = input.children.map((child) => {
    ...
    const allocation = child.eventAllocatedAmountMnt > 0
      ? `<br>Энэ удаагийн хуваарилалт: <strong>${escapeHtml(mnt(child.eventAllocatedAmountMnt))}</strong>` : "";
    return `<section><p><strong>${escapeHtml(child.childName)}</strong><br>Анги: ${escapeHtml(formatClassDisplay(child.classLabel))}${allocation}<br>Нийт төлсөн дүн: ${escapeHtml(mnt(child.totalPaidAmountMnt))}<br>Төлөх үлдэгдэл: <strong>${escapeHtml(mnt(child.remainingAmountMnt))}</strong>${next}</p><p><strong>${child.seatConfirmed ? "Суудал баталгаажсан." : "Суудал хараахан баталгаажаагүй байна."}</strong></p>${group}</section>`;
  ...
  const textItems = input.children.map((child) => {
    ...
    const allocation = child.eventAllocatedAmountMnt > 0 ? `\nЭнэ удаагийн хуваарилалт: ${mnt(child.eventAllocatedAmountMnt)}` : "";
    return `${child.childName}\nАнги: ${formatClassDisplay(child.classLabel)}${allocation}\nНийт төлсөн дүн: ${mnt(child.totalPaidAmountMnt)}\nТөлөх үлдэгдэл: ${mnt(child.remainingAmountMnt)}${next}\n${child.seatConfirmed ? "Суудал баталгаажсан." : "Суудал хараахан баталгаажаагүй байна."}${group}`;
  }).join("\n\n");
  ...
  return {
    ...
    html: `<!doctype html><html lang="mn"><body style="margin:0;padding:24px;background:#f8fafc;color:#172033;font-family:Arial,sans-serif;line-height:1.5"><div style="max-width:560px;margin:0 auto;background:#fff;padding:24px;border:1px solid #dfe4ec;border-radius:8px"><h1 style="margin:0 0 16px;color:#052b83;font-size:24px">Төлбөр баталгаажлаа</h1><p>Энэ удаа хүлээн авсан төлбөр: <strong>${escapeHtml(mnt(input.eventReceivedAmountMnt))}</strong></p><p>Доорх нь тухайн төлбөрийн хуваарилалт болон энэ мэдэгдэл үүсэх үеийн нийт төлөлтийн дүн юм.</p>${items}${center}</div></body></html>`,
    text: `Наран Эрдэм — Төлбөр баталгаажлаа\n\nЭнэ удаа хүлээн авсан төлбөр: ${mnt(input.eventReceivedAmountMnt)}\nДоорх нь тухайн төлбөрийн хуваарилалт болон энэ мэдэгдэл үүсэх үеийн нийт төлөлтийн дүн юм.\n\n${textItems}${textOnboarding}`,
  };
}
