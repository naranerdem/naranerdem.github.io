...
Parent-facing account access is passwordless and based on verified communication channels. A system-delivered enrollment email may carry a one-time `Бүртгэлээ харах` link: successful use proves control of that email address, records the existing email-verification fact, and creates a short-lived parent session. The v1 parent surface is read-only and retrieves only records joined through the exact canonical guardian linkage. A copied staff-generated manual message contains no such link and never verifies email or phone. Future SMS follows the same channel-bound rule without treating email as the permanent only channel.
...
The temporary staging-only submission restriction was intentional while public writes were rehearsed: it accepted only explicit test fixtures and marked every resulting draft, hold, payment, and verification row as test data. Production-safe submission instead derives provenance from a consistent selected Offering/ClassSession source. Production accepts only non-test Offering, academic-year, and class configuration; test fixtures are rejected server-side even when their IDs are posted directly. Staging may exercise either explicit test fixtures or ordinary non-test-like rehearsal configuration, but one accepted draft never mixes the two provenances.

Migrations 0037/0038 add referral capture and provenance. A confirmed child/enrollment receives one opaque, case-insensitive referral code with no PII. A submitted code must resolve to an active confirmed source and is captured against the draft child; canonical promotion creates the canonical referral relationship. Same-child or same-guardian referrals are preserved as audited `disqualified` relationships. Migration 0039 adds the typed 10% family, 5% successful-referrer, and 2% referred-child policy plus a durable `discount_award` ledger. The referred child award is created at accepted registration; the referrer award is created once only after the referred child is confirmed. Awards retain their own rate, chosen-plan base, MNT amount, reason, provenance, reversal, and audit record; changing policy never reprices history.

Effective payment obligations are projected from immutable selected-plan and installment snapshots plus active discount awards. Awards stack as independent integer-MNT reductions of the selected plan total and apply initial installment first, then later installments. Received payments and raw plan snapshots are never rewritten. When a later award exceeds what remains unpaid, its unused portion is recorded as an explicit discount-credit position for staff reconciliation rather than producing a negative installment. Family qualification is based on children registered together in one accepted submission or on a later canonical guardian relationship, never on matching contact text. A family dropping below two children does not automatically claw back an awarded discount; an admin must make any exceptional reversal through the audited ledger.
...
For a ClassSession, capacity consumption is the set of confirmed enrollments, unresolved initial-payment reservations, active waitlist offers, and any genuinely active legacy `awaiting_initial_payment` enrollment, counted once per child/seat. Current accepted registrations create the draft-hold representation, not the legacy enrollment form. Promotion atomically releases that hold while creating the confirmed enrollment, so the two representations do not overlap in an observable state; a defensive projection exclusion also prevents a draft hold linked to a canonical enrollment from consuming twice. A plain FIFO waitlist entry never consumes capacity. An active offer has its own snapshotted response target (default 1,440 minutes / 24 hours), remains capacity-consuming after that target, and is released only by an explicit parent/staff decision. Offer acceptance atomically changes the representation to an ordinary initial-payment reservation without a free-seat gap; an explicit unresolved-unpaid release restores one seat and then allocates the next eligible FIFO offer. A finalized teacher-approved partial confirmation remains a confirmed seat even when its remaining balance later becomes overdue. A referenced class may change its configured capacity only at or above its current consumed count; its conditional D1 update re-evaluates that count at write time, and an increase creates any FIFO offers in the same D1 transaction so unrelated public registration cannot bypass the queue.

Teacher/admin may explicitly cancel an accepted registration before attendance history exists. Cancellation is terminal: it marks the draft child and any canonical enrollment cancelled, resolves the active hold or confirmed-enrollment representation, releases exactly one seat, stops pending collection reminders, and immediately runs the existing FIFO offer allocator for that class. A replay releases no additional seat. Received payments remain financial history; an unshared received amount becomes available credit rather than an automatic refund, while a shared payment is left for explicit reconciliation. A tentative unshared confirmation is ended before Cron finalization. Attendance, absence, or make-up history blocks this simple operation and requires a later withdrawal workflow.
...
Migration 0064 keeps the minute scheduler proportional to changed work. Payment-credit and transfer-credit writes enqueue a canonical learner for child-credit reconciliation; installment, confirmation, and registration-status writes enqueue the affected payment request for milestone reconciliation. Each queue uses a revision-guarded lease and indexed batches, while a one-time cursor sweep safely adopts pre-0064 history without treating every minute as a history scan. Child-level credit review still reconciles synchronously before a reminder decision, so queued background work cannot make an otherwise current credit balance look overdue.
...
Public login-start responses are generic for active, unknown, guardian-only, and disabled addresses. Only an active staff account can queue email. Staff login uses its own server-verified Turnstile boundary and Worker rate-limit binding before email work, while per-email and per-IP hourly limits plus a resend cooldown remain defense in depth. Fake staging staff continue to use the mandatory safe-recipient override. An explicitly provisioned, non-test staging staff identity may receive its own login email; public/parent test mail and fake staff never bypass the override. Production staff email remains disabled until its separate real Turnstile credentials, sender/provider setup, and rate-limit binding are configured and rehearsed.
...
Do not send redundant emails for every internal technical state transition. `EMAIL_ENABLED` governs ordinary transactional parent messages, including the accepted-registration receipt, payment instructions, enrollment confirmation, payment confirmation, and payment reminders. The enrollment-confirmation message can include a channel-bound parent-access link without enabling the optional explicit-auth flow governed by `AUTH_EMAIL_ENABLED`. A transactional delivery failure is recorded in `outbound_email` but never undoes an accepted registration, its payment request, canonical enrollment, or capacity reservation.

Staging tests must exercise ordinary transactional email as well as the separate real challenge, Mongolian verification template, outbound-email, provider, and session path. `STAGING_EMAIL_OVERRIDE_TO` is mandatory whenever staging sending is enabled: `intended_to_email` preserves the entered address and `actual_delivery_email` is always the controlled inbox. Missing override configuration fails closed. The staging auth-start route also requires `STAGING_AUTH_TEST_KEY` in the `X-Naran-Test-Key` header; the key is never placed in browser JavaScript or URLs, and the bypass does not exist in production.

`outbound_email` is also the authoritative basis for the admin-only sanitized `Илгээсэн и-мэйл` Outbox. A queue/send attempt stores an immutable safe display snapshot from the message being submitted; capability URLs are replaced server-side before persistence and are never returned to the browser. A narrow audited admin setting may configure two internal recipient lists: the admin list receives every archive-safe operational event, while the teacher list receives only the `registration_received` event and a separate final-confirmation notice. Routing is based on durable event types, not translated subjects; overlapping internal addresses and the primary recipient are deduplicated. The list is resolved when an unsnapshotted Outbox item is first delivered and then persisted in `bcc_recipients_json`, so provider retries keep the same recipients while later configuration changes affect only still-unsnapshotted messages. Capability-bearing login, verification, challenge, waitlist-offer, and parent `enrollment_confirmed` messages are not raw-BCC'd. On first successful final confirmation, `internal_enrollment_confirmed` instead creates one token-free internal notice with the enrollment/class and settlement summary; it has its own idempotency key, creates no challenge, and does not recursively receive internal copies. Internal delivery remains advisory and cannot roll back canonical enrollment. BCC is supplementary provider evidence and cannot change application truth. In staging, the normal safe-recipient override is used for the direct internal notice and the configured archive override replaces both lists for archive-safe parent deliveries, so rehearsal mail cannot leak to configured recipients.

Public seat counts are a presentation policy, never a capacity input. The typed singleton `public_seat_count_threshold_setting` has a nullable legacy default that continues showing all available counts. An explicit nonnegative threshold `N` exposes the authoritative free-seat count only when `N > 0` and free seats are at most `N`; `N = 0` exposes no numeric count. Public catalog responses retain the nonnumeric `available`, `full`, or `unavailable` status and omit suppressed counts. Staff catalog projections retain the count, while submission continues to perform the existing atomic capacity write regardless of any displayed value.

Registration submit and email-change operations require Cloudflare Turnstile and validate every token server-side with Siteverify; missing, invalid, replayed, expired, and network-failed validation all fail closed. Staging accepts only Cloudflare's published test credentials, while production requires separate real credentials and rejects the test pair. The public browser receives only a site key. Before production registration is enabled, the owner must create a production widget restricted to the production hostname, store its secret as a Worker secret, retain generic non-enumerating errors, and complete the operational rehearsal. A client-only bot check is never sufficient.
...
When an explicitly released reservation has received money, its received-payment history is retained and the unconsumed amount becomes explicit `payment_credit`. Staff can record a partial refund or transfer an explicitly reviewed amount to another eligible canonical child through one audited ledger operation; the original receipt and allocation are never copied or rewritten. The remaining refundable amount is shared atomically by refund and transfer, while discount/award credit remains a separate non-refundable child-ledger source. Matching text contact data is never sufficient for automatic cross-registration allocation.
