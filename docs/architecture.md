...
Do not send redundant emails for every internal technical state transition. `EMAIL_ENABLED` governs ordinary transactional parent messages, including the accepted-registration receipt, payment instructions, payment confirmation, and payment reminders. `AUTH_EMAIL_ENABLED` independently governs only verification, login, and other capability-bearing parent email. A transactional delivery failure is recorded in `outbound_email` but never undoes an accepted registration, its payment request, or its capacity reservation.
