...
`outbound_email` is also the authoritative basis for the admin-only sanitized `Илгээсэн и-мэйл` Outbox. A queue/send attempt stores an immutable safe display snapshot from the message being submitted; capability URLs are replaced server-side before persistence and are never returned to the browser. A narrow audited admin setting may configure an internal BCC archive list for explicitly classified ordinary parent messages only. Capability-bearing login, verification, challenge, and waitlist-offer messages are not BCC'd. BCC is supplementary provider evidence and cannot change application truth. In staging, archive BCC is disabled unless an explicitly configured staging-safe archive override exists, so the mandatory parent safe-recipient override cannot leak rehearsal mail.
