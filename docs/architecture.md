...
Migration 0037 adds the small launch referral-capture foundation. A confirmed child/enrollment receives one opaque, case-insensitive referral code with no PII. A submitted code must resolve to an active confirmed source and is captured against the draft child; canonical promotion creates the pending referral relationship. Same-child or same-guardian referrals are preserved as audited `disqualified` relationships, never as future discount candidates. The parent sees the new code only after confirmation. No discount, repricing, or automatic credit is calculated by this foundation.
