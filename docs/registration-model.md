...
Before attendance history exists, teacher/admin may instead explicitly use `Бүртгэлийг цуцлах` for a guardian request, overdue-payment decision, or recorded other reason. It is terminal for this version: the registration child and any canonical enrollment become cancelled, the capacity representation is resolved, exactly one seat is released, and the normal FIFO waitlist allocator runs immediately. Cancelling never deletes child, guardian, payment, discount, or audit history, and it never invents an automatic refund. Unshared received funds become available credit for later staff reconciliation; shared payments remain intact for an explicit allocation decision. Pending payment reminders and an unshared tentative confirmation are stopped so Cron cannot later promote the cancelled child. If attendance, absence, or make-up history exists, ordinary cancellation is blocked; a later audited withdrawal/settlement lifecycle is required instead.
...
After form completion, the parent immediately sees class/waitlist state, saved payment amount, deadline, and bank instructions; the same registration-scoped information is sent as an ordinary transactional receipt when email delivery is enabled. Optional email verification, when enabled separately, is presented only as a future convenience for viewing or managing registration information. Resend has a 60-second cooldown for capability-email resend; it never restarts the payment deadline. An HttpOnly draft-access cookie restores this registration-scoped status without refilling the form.
...
Current lifecycle findings, captured by the disposable `scripts/registration-hold-test.mjs` regression, are intentionally not a new financial policy: awards for three children accepted together are created at accepted submission, before payment or canonical promotion; cancelling down from three qualifying records to two and then one does not automatically reverse those historical awards. Effective ordinary award projection distributes reductions across installments in initial-then-later order. The explicit confirmed-family recovery path can materialize a fully paid award excess as child-owned residual credit, but the ordinary same-guardian promotion path does not yet provide the same automatic residual-credit recovery. Any future rule that removes qualification after cancellation/expiry, creates new debt, reverses already-used value, or repairs residual credit needs an approved, fenced reconciliation policy rather than an implicit recalculation.
...
For a child-credit settlement, credit is eligible for a one-payment agreement or the final scheduled installment of a multi-installment agreement. It is never applied to the first installment of a two-installment agreement. This is derived from the immutable selected-plan snapshot and installment identity, not from an inferred balance. Historical credit applications remain immutable even where the current policy would no longer allow a new first-installment application. A pending additional-class admission may reserve an explicitly teacher-confirmed amount of existing child credit for its eligible target obligation. The reservation changes neither the obligation nor received cash and is released on expiry or cancellation. A source award that has not yet been earned is recorded only as a contingent reservation; it becomes a ledger root and may be applied only by the fenced target-confirmation continuation. Existing and contingent credit remain separate until that continuation completes. It revalidates the admission and settlement under a durable claim, then atomically applies credit, converts the hold, and creates the canonical enrollment. Replay and finalizer retry reuse that fenced state; a failed continuation preserves any real cash receipt without treating contingent credit as paid. A confirmed reservation is then represented by ordinary credit-application ledger entries, preserving raw agreement installments and received-payment history. Withdrawal reconciliation and automatic cross-class discount or credit reversal remain deliberately deferred.

An established child may also receive an additional class from one teacher-selected child entry in an existing ordinary incoming registration. This is a staff-confirmed incorporation, not a general identity merge: it retains the incoming draft, pricing snapshot, hold, payment request, and raw contact history; associates only the chosen child with the established canonical child; and creates the same fenced additional-class admission used by a staff-created target. The teacher manually searches the current academic year's incoming children, reviews both identities, requested class, original terms, preserved referral value, and resulting family discount, then confirms both acknowledgements. Selection and preview make no mutation. Automatic matching, dismissed-match history, and dashboard notifications are deliberately deferred.

The incorporation accepts an unpaid, uncredited, unpromoted entry with an active initial-payment hold and compatible academic-year/test provenance. A captured referral and its earned referred-child award remain attached to that child and combine additively with the new additional-class family entitlement under the existing allocation rules: family/additional-class value reduces the final unpaid installment first, while ordinary referral value is allocated across remaining unpaid installments. An already-earned family award, payment, credit use, canonical ownership, missing hold, or duplicate current class is unsupported by this flow; it makes no change to the incoming record and exposes no separate financial-review action. Ordinary multi-child submissions receive conditional quotes, not earned family awards, until their protected funding finalizer qualifies them, so an otherwise unpaid child with only a pending quote remains eligible for this incorporation. A matching pending conditional-family quote is cancelled only for the incorporated child and replaced by the same-priced, fenced additional-class promise, leaving siblings' drafts and quotes untouched.

Public referral codes remain child-level canonical identities, while referral-benefit attribution is agreement-specific and durable. A code has one owning enrollment; acceptance captures that exact enrollment in `registration_draft_referral`, promotion persists it in `referral`, and the referrer-side award is uniquely keyed to that referral. A second class therefore reuses the child's existing public code but neither moves nor copies prior benefits, and a later distinct referral creates one additional award on the originally captured agreement. Incorporation preserves a referred child's captured code and referred-child award under the same additive allocation rules; finalizer replay cannot duplicate either side. No current path selects a referring agreement by `ORDER BY` or fallback. Percentage splitting across classes is deferred.
