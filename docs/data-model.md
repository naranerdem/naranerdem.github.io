...
- `registration_capacity_hold`: one per draft child. New `initial_payment` rows snapshot the typed initial-payment deadline at accepted submission; historical `provisional_email_confirmation` rows remain compatible. A payment deadline never automatically releases an initial reservation.
...
Migration `0050_internal_email_copy_recipients.sql` retains the existing `email_archive_bcc_setting.recipients_json` value as the admin list and adds an empty teacher list. The two lists together allow at most five distinct normalized internal addresses. The admin list receives every archive-safe operational event; the teacher list receives the initial `registration_received` event plus the separate capability-free final-enrollment notice. Login, verification, one-time challenge, waitlist-offer, and parent-access enrollment-confirmation messages are never copied verbatim. The archive is secondary evidence, not payment, capacity, enrollment, or waitlist truth.
...
Migration `0029_payment_confirmation_grace.sql` adds a typed singleton grace-duration setting and a durable per-payment confirmation record. The record snapshots `finalize_after`, optional teacher-approved partial-seat status, and a remaining-payment deadline; changing the setting never rewrites an existing deadline. Migration `0030_payment_credit_refund.sql` records available credit created by an explicit release after money was received. Both retain payment/evidence history and do not create a one-enrollment-per-student constraint.

Migration `0031_initial_payment_deadline_setting.sql` adds the independent typed singleton `initial_payment_deadline_setting`, defaulting to 1,440 minutes. It affects only future accepted registration holds; later-installment and teacher-approved remaining-payment deadlines remain independent snapshots.

Migration `0050_internal_email_copy_recipients.sql` keeps the existing internal-recipient JSON as the admin list and adds an independent teacher-recipient JSON list. Lists are administrator-only, parsed as comma/LF/CRLF-separated addresses, and limited to five distinct addresses together. Migration `0051_public_seat_count_threshold.sql` adds a typed nullable singleton: `NULL` explicitly preserves legacy public count display, while a stored integer `0..1000` controls when public catalog projections include a free-seat number. Neither setting changes capacity, payments, registrations, or lifecycle records.
...
Migration `0017_staff_email_aliases.sql` adds `staff_account_email` as the login-address authority. It backfills every existing account address as primary, permits at most three globally unique normalized addresses per account, and keeps `staff_account.email_normalized` as a synchronized compatibility mirror of the primary address. Challenges retain the exact address used, but sessions remain attached to the staff account. Alias addition and primary changes therefore preserve sessions; alias removal invalidates pending access for only that address.
