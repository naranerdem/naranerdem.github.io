...
assert.match(page, /id="payment-copy" class="button secondary"/, "Excel copy shares the outlined toolbar treatment with refresh and export");
assert.match(page, /id="payment-copy"[^>]*aria-label="Excel-д хуулах"[^>]*disabled>Бэлтгэж байна…</, "clipboard copy starts disabled while the authorized report is prepared");
...
assert.match(page, /void prepareRegistrationReport\(\)/, "each list refresh begins preloading the authorized clipboard projection");
assert.match(page, /button\.textContent = ready \? "Хуулах" : failed \? "Хуулах" : "Бэлтгэж байна…"/, "clipboard copy remains disabled with stable preparation feedback until ready");
const copyHandler = page.slice(page.indexOf('q("#payment-copy").addEventListener'), page.indexOf('q("#payment-export").addEventListener'));
assert.match(copyHandler, /const report = state\.registrationReport/, "clipboard handler uses the already-authorized cached report");
assert.match(copyHandler, /copyReportToClipboard\(report\)/, "clipboard write begins directly from the initiating click");
assert.doesNotMatch(copyHandler, /registrationReport\(\)|fetch\(/, "clipboard handler does not begin an export fetch before copying");
assert.match(page, /Хуулах зөвшөөрөл олгогдсонгүй\. Доорх текстийг гараар хуулна уу\./, "clipboard denial exposes a concise Mongolian manual-copy fallback");
assert.match(page, /message\("Хуулагдлаа\."/, "Excel copy confirms success inline");
