...
assert.match(page, /Дахин бүртгүүлэх шаардлагагүй/, "post-commit recovery explicitly prevents duplicate resubmission");
assert.match(page, /turnstileArea\.scrollIntoView\(\{ behavior: "smooth", block: "center" \}\)/, "a rejected submission returns the parent to its visible error message");
assert.match(router, /Анги, цагаа дахин сонгоно уу\. Сонгосон анги өөрчлөгдсөн байж магадгүй\./, "stale or provenance-invalid class choices receive an actionable response");
...
assert.match(page, /turnstile\.render\(turnstileWidget, \{[\s\S]*?appearance: "interaction-only"/, "the managed Turnstile widget stays hidden after a background challenge while remaining available when interaction is required");
