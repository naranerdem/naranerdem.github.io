...
try {
  ...
  const registrationWindowsPage = readFileSync("src/pages/staff/registration-windows.astro", "utf8");
  ...
  assert.match(schedulePage, /slot\.status === "scheduled" \? escape\(slot\.lessonTitle\) : escape\(slot\.reasonLabel \|\| "Хичээлгүй"\)/, "only a scheduled slot renders a lesson title; no-class rows render their authoritative reason");
  assert.match(schedulePage, /slot\.status === "scheduled" \? slot\.lessonSequence \|\| "–" : "–"/, "only a scheduled slot renders a lesson ordinal");
  assert.doesNotMatch(schedulePage, /cancelledLessonTitle\)} · Хичээлгүй/, "a cancelled slot does not visibly retain its displaced lesson title");
  assert.match(schedulePage, /staff-calendar-no-lesson/, "no-class rows receive the muted no-lesson treatment");
  assert.match(styles, /\.staff-calendar-exception \.staff-order,\s*\.staff-calendar-no-lesson/, "the no-lesson ordinal and reason use a subtle shared accent treatment");
  assert.match(paymentsPage, /class="staff-payment-page-title"/, "the payment page title has a dedicated one-line responsive treatment");
  assert.match(styles, /\.staff-payment-page-title \{\s*white-space: nowrap;/, "the payment page title remains on one line without changing public headings");
  assert.match(registrationWindowsPage, /staff-form-grid staff-window-date-grid/, "registration-window dates use a dedicated responsive grid");
  assert.match(styles, /\.staff-window-date-grid \{\s*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/, "registration-window dates remain two columns when space permits");
  assert.match(styles, /@media \(max-width: 38rem\)[\s\S]*?\.staff-window-date-grid \{\s*grid-template-columns: minmax\(0, 1fr\);/, "registration-window dates stack before native date controls can collide on narrow screens");
