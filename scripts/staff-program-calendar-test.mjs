...
try {
  ...
  await offeringService.saveActivityOffering(runtime, actor("teacher"), { kind: "annual_course", academicYearId: "year-2025", stageCode: "stage_1", startsOn: "2025-09-01", endsOn: "2026-06-01", curriculumProgramId: "old-program" });
  ...
  await offeringService.saveActivityOffering(runtime, actor("teacher"), { kind: "summer_course", title: "Туршилтын зуны сургалт", startsOn: "2027-06-01", endsOn: "2027-06-26", facebookGroupUrl: "https://facebook.com/groups/fake-summer" });
  ...
  const summerDraft = database.query(`SELECT id, updated_at AS updatedAt FROM curriculum_program WHERE id = ${quote(summerOffering.programId)}`)[0];
  const summerLessons = Array.from({ length: 12 }, (_, index) => ({ title: `Туршилтын хичээл ${String(index + 1).padStart(2, "0")}` }));
  ...
  await service.publishProgramDraft(runtime, actor("teacher"), { programId: summerDraft.id, expectedUpdatedAt: savedSummerDraft.updatedAt, offeringId: summerOffering.id });
  ...
  await service.saveClassSession(runtime, actor("teacher"), { offeringId: summerOffering.id, recurrenceKind: "weekdays", firstDate: "2027-06-01", lastDate: "2027-06-14", academicYearId: "", stageCode: "", weekday: "", startTime: "10:00", endTime: "11:30", capacity: 12 });
  const summerWeekdayClass = database.query("SELECT id, updated_at AS updatedAt FROM class_session WHERE activity_offering_id = " + quote(summerOffering.id) + " AND start_time = '10:00'")[0];
  await assert.rejects(() => service.generateCalendarDraft(runtime, actor("teacher"), { classSessionId: summerWeekdayClass.id }), (error) => error.code === "insufficient_slots", "a 12-lesson weekday program cannot escape a short hard period");
  await service.saveClassSession(runtime, actor("teacher"), { id: summerWeekdayClass.id, expectedUpdatedAt: summerWeekdayClass.updatedAt, offeringId: summerOffering.id, recurrenceKind: "weekdays", firstDate: "2027-06-01", lastDate: "2027-06-16", academicYearId: "", stageCode: "", weekday: "", startTime: "10:00", endTime: "11:30", capacity: 12 });
  await service.generateCalendarDraft(runtime, actor("teacher"), { classSessionId: summerWeekdayClass.id });
  assert.equal(count(database, "class_calendar_slot", "local_date = '2027-06-07' AND status = 'scheduled'"), 1, "an academic break does not suppress a summer class");
  await service.saveClassSession(runtime, actor("teacher"), { offeringId: summerOffering.id, recurrenceKind: "daily", firstDate: "2027-06-15", lastDate: "2027-06-26", academicYearId: "", stageCode: "", weekday: "", startTime: "13:00", endTime: "14:30", capacity: 12 });
  ...
  assert.equal(database.query(`SELECT COUNT(*) AS count FROM class_calendar_slot AS slot INNER JOIN class_calendar_revision AS revision ON revision.id = slot.class_calendar_revision_id INNER JOIN class_calendar AS calendar ON calendar.id = revision.class_calendar_id WHERE calendar.class_session_id = ${quote(summerDailyClass.id)} AND slot.local_date BETWEEN '2027-06-15' AND '2027-06-26' AND slot.status = 'scheduled'`)[0].count, 12, "daily summer recurrence uses every local civil date");
